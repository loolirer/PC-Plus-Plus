...
vector<string> Loja::imprimirEstoque(int tipoProduto) {
	...
	for (unsigned i = 0; i < produtos.size(); i++) {
		switch (tipoProduto) {
			case PROCESSADOR:
				if (Processador *processador = dynamic_cast<Processador*>(produtos[i])) {
					produtosListados.push_back(produtos[i]->getID());
					processador->imprimirProduto(index++);
				}

				break;
			case PLACAMAE:
				if (PlacaMae *placaMae = dynamic_cast<PlacaMae*>(produtos[i])) {
					produtosListados.push_back(produtos[i]->getID());
					placaMae->imprimirProduto(index++);
				}

				break;
			case MEMORIARAM:
				if (MemoriaRAM *memoriaRAM = dynamic_cast<MemoriaRAM*>(produtos[i])) {
					produtosListados.push_back(produtos[i]->getID());
					memoriaRAM->imprimirProduto(index++);
				}

				break;
			case PLACADEVIDEO:
				if (PlacaDeVideo *placaDeVideo = dynamic_cast<PlacaDeVideo*>(produtos[i])) {
					produtosListados.push_back(produtos[i]->getID());
					placaDeVideo->imprimirProduto(index++);
				}
			case MEMORIASECUNDARIA:
				if (MemoriaSecundaria *memoriaSecundaria = dynamic_cast<MemoriaSecundaria*>(produtos[i])) {
					produtosListados.push_back(produtos[i]->getID());
					memoriaSecundaria->imprimirProduto(index++);
				}

				break;
			case GABINETE:
				if (Gabinete *gabinete = dynamic_cast<Gabinete*>(produtos[i])) {
					produtosListados.push_back(produtos[i]->getID());
					gabinete->imprimirProduto(index++);
				}

				break;
			case FONTEDEALIMENTACAO:
				if (FonteDeAlimentacao *fonteDeAlimentacao = dynamic_cast<FonteDeAlimentacao*>(produtos[i])) {
					produtosListados.push_back(produtos[i]->getID());
					fonteDeAlimentacao->imprimirProduto(index++);
				}
...
void Loja::serializar(Produto *produto, ostream &os) {
	...
	Serializador::serializarString(nome, os);
